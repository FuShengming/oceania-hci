...
public interface WorkPlaceRepository extends JpaRepository<WorkSpace, Integer> {
    @Query(value = "select * from work_space where code_id=?1 order by work_space_date desc limit 1", nativeQuery = true)
    public WorkSpace findLatestWorkSpace(int codeId);
